...
ArticleAnimator.contentizeElement = function($el, d){
  ...
  $el.find('h1.blog-title').html(d.title);
  $el.find('h2.description').html(d.title_secondary);
  $el.find('.content .text').html(d.content);
  $el.find('div.blog-detail time').html(d.date);
  $el.find('ul.dec-list time').html(d.date_project);
  $el.find('ul.dec-list client').html(d.client);
  $el.find('ul.dec-list adresse').html(d.adresse);
  $el.find('div.blog-detail author').html(d.author);
  $el.find('ul.dec-list category').html(d.category);
  $el.find('projectbtn').html(d.link_project);
  $el.find('div.blog-image').html(d.start);
  $el.find('div.post-lightbox').html(d.gallery);
}
